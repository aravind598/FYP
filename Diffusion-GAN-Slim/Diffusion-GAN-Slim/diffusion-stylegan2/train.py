...
def setup_training_loop_kwargs(
    ...
    cfg_specs = {
        ...
        'autov2':    dict(ref_gpus=1,  kimg=50000,  mb=32, mbstd=4,  fmaps=1,   lrate=0.002,  gamma=5,    ema=10,  ramp=0.05, map=2), # Anime
        'autov3':    dict(ref_gpus=1,  kimg=25000,  mb=32, mbstd=4,  fmaps=2,   lrate=0.002,  gamma=5,    ema=10,  ramp=0.05, map=8), # Anime
        'stylegan2': dict(ref_gpus=1,  kimg=25000,  mb=32, mbstd=4,  fmaps=1,   lrate=0.002,  gamma=10,   ema=10,  ramp=None, map=8), # Uses mixed-precision, unlike the original StyleGAN2.
    ...
    args.G_kwargs.synthesis_kwargs.channel_max = args.D_kwargs.channel_max = 512 #1024 too big 200M Params
...
# Base config.
@click.option('--cfg', help='Base config [default: auto]', type=click.Choice(['auto', "autov2", "autov3",'stylegan2', 'paper256', 'paper512', 'paper1024', 'cifar', 'stl10']))
...
#----------------------------------------------------------------------------
#python train.py --outdir=training-runs --data="/workspace/Diffusion-GAN/512-anime-16K.zip" --gpus=1 --batch 32 --cfg autov3 --kimg 25000 --aug diff --target 0.6 --noise_sd 0.05 --ts_dist priority
